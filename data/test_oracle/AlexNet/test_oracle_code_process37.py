...
class TestNeuronEffectBlock(unittest.TestCase):
    ...
    def test_neuron_effect_block(self):
        ...
        self.assertEqual(mutated_model.fc.weight.sum(), 0, "Activations should be removed after removing activations.")

        # Replace Activations
        mutated_model = neuron_effect_block(self.model, proportion=0.1)
        mutated_model = replace_activations(mutated_model)
        self.assertEqual(mutated_model.fc.weight.sum(), 0, "Activations should be replaced after replacing activations.")

        # Uniform Fuzzing
        mutated_model = neuron_effect_block(self.model, proportion=0.1)
        mutated_weights = uniform_fuzz_weight(mutated_model, min_val=-0.1, max_val=0.1)

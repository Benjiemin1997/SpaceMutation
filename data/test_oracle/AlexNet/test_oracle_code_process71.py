...
class TestReplaceActivations(unittest.TestCase):
    ...
    def test_replace_activations(self):
        ...
        # Verify that the new activations are different from the original ones
        self.assertIsNotNone(mutated_model[0])
        self.assertIsNotNone(mutated_model[1])
        self.assertIsNotNone(mutated_model[2])
        self.assertIsNotNone(mutated_model[3])
        self.assertIsNotNone(mutated_model[4])
        self.assertIsNotNone(mutated_model[5])
        self.assertIsNotNone(mutated_model[6])
        self.assertIsNotNone(mutated_model[7])

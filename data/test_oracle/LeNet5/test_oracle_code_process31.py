...
class TestNeuronEffectBlock(unittest.TestCase):
    ...
    def test_neuron_effect_block(self):
        ...
        # Random shuffle
        mutated_model_shuffle = random_shuffle_weight(self.model)
        self.assertNotEqual(self.model.state_dict(), mutated_model_shuffle.state_dict())

        # Remove activations
        mutated_model_remove = remove_activations(self.model)
        self.assertNotEqual(self.model.state_dict(), mutated_model_remove.state_dict())

        # Replace activations
        mutated_model_replace = replace_activations(self.model)
        self.assertNotEqual(self.model.state_dict(), mutated_model_replace.state_dict())

        # Uniform fuzzing
        mutated_model_uniform = uniform_fuzz_weight(self.model)
        self.assertNotEqual(self.model.state_dict(), mutated_model_uniform.state_dict())

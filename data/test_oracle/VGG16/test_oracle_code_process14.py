...
def test_random_shuffle_weight():
    ...
    # Perform the random shuffle on the model's weights
    shuffled_model = random_shuffle_weight(model)

    # Check if all parameters have been shuffled
    ...
    for original_param_id, original_param in original_params.items():
        ...
        try:
            torch.testing.assert_allclose(original_param, shuffled_param, atol=1e-4, rtol=1e-4)
        except AssertionError as e:
            print(f"Parameter at id {original_param_id} did not match after shuffling.")
            raise e
    print("All parameters shuffled successfully.")
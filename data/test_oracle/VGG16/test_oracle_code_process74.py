...
def test_random_shuffle_weight():
    ...
    for original_param, new_param in zip(model.parameters(), new_model.parameters()):
        assert not torch.allclose(original_param, new_param), "Model parameters should not be identical after shuffling"


    for param in new_model.parameters():
        assert not torch.equal(param, torch.zeros_like(param)), "Parameters should have been shuffled"

    print("Test passed: random_shuffle_weight function works correctly.")
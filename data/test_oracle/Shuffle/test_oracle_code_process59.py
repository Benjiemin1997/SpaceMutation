...
def test_neuron_effect_block():
    ...
    assert model.linear.weight.shape[1] != model.linear.weight.shape[1], "Linear layer weights should be altered."
    
    # Test neuron effect block on Conv2d layer
    model = neuron_effect_block(model, proportion=0.5)
    assert model.conv.weight.shape[0] != model.conv.weight.shape[0], "Conv2d layer weights should be altered."

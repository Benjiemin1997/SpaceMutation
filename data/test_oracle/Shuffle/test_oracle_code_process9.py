...
def test_neuron_effect_block():
    ...
    for name, param in model.named_parameters():
        if 'linear' in name:
            assert torch.sum(param) == 0, f"Linear layer parameters were not modified properly."
    ...
    for name, param in model.named_parameters():
        if 'conv' in name:
            assert torch.sum(param) == 0, f"Conv2d layer parameters were not modified properly."

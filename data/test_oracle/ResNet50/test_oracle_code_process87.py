...
def test_neuron_effect_block():
    ...
    assert not torch.equal(mutated_model.linear.weight, torch.zeros_like(mutated_model.linear.weight)), "Linear layer weights should be mutated."
    assert not torch.equal(mutated_model.conv.weight, torch.zeros_like(mutated_model.conv.weight)), "Conv2d layer weights should be mutated."

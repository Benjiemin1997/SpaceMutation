...
def test_fgsm_fuzz_weight():
    ...
    model_gaussian_fuzzed = random_shuffle_weight(model)
    assert torch.allclose(model_gaussian_fuzzed.conv1.weight, model.conv1.weight + torch.randn_like(model.conv1.weight) * epsilon, atol=1e-3)
    ...
    model_uniform_fuzzed = uniform_fuzz_weight(model, epsilon)
    assert torch.allclose(model_uniform_fuzzed.conv1.weight, model.conv1.weight + torch.rand_like(model.conv1.weight) * (2*epsilon) - epsilon, atol=1e-3)

...
def test_random_shuffle_weight():
    ...
    assert not all(torch.equal(original, shuffled) for original, shuffled in zip(original_state.values(), shuffled_state.values())), "Weights have not been shuffled."
    ...
    assert not torch.allclose(output_before, output_after), "The model output after shuffling weights should be different from before."

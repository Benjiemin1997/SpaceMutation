    ...
    def test_replace_activations(self, mock_model):
        ...
        for _, module in mock_model_instance.named_modules():
            if isinstance(module, nn.ReLU):
                ...
                self.assertEqual(type(module), nn.LeakyReLU)  # Example of a randomly chosen activation
            elif isinstance(module, nn.LeakyReLU):
                self.assertNotEqual(type(module), nn.LeakyReLU)
                self.assertEqual(type(module), nn.ReLU)  # Reverting back to original activation for verification

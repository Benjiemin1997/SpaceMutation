...
class TestUniformFuzzWeight(unittest.TestCase):
    ...
    def test_uniform_fuzz_weight(self):
        ...
        original_params = {name: param.clone().detach().requires_grad_(True) for name, param in
                           self.model.named_parameters()}
        mutated_params = {name: param.clone().detach().requires_grad_(True) for name, param in
                          mutated_model.named_parameters()}

        any_param_changed = any(
            not torch.allclose(original_params[name], mutated_params[name]) for name in original_params.keys())

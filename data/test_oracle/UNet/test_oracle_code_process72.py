...
class TestReplaceActivations(unittest.TestCase):
    ...
    def test_replace_activations(self, mock_model):
        ...
        result_model = replace_activations(mock_model_instance)

        # Assert that the correct activation was replaced
        self.assertNotEqual(result_model.downs[0].conv[0][0], nn.ReLU())
        self.assertEqual(result_model.downs[0].conv[0][0], nn.ELU(alpha=1.0))


        self.assertEqual(result_model.downs[0].conv[1], nn.BatchNorm2d(64))
        self.assertEqual(result_model.downs[0].conv[2], nn.Identity())
        self.assertEqual(result_model.downs[0].conv[3], nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)))
        self.assertEqual(result_model.downs[0].conv[4], nn.BatchNorm2d(64))
        self.assertEqual(result_model.downs[0].conv[5], nn.Identity())

        self.assertNotEqual(result_model.state_dict()['downs.0.conv.0.0.weight'], mock_model_instance.state_dict()['downs.0.conv.0.0.weight'])

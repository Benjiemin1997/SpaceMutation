...
# =========================================================
# Temperature / Usage Sampling
# =========================================================
def get_cpu_temperature() -> Optional[float]:
    """
    Read CPU temperature from common Linux thermal zone.
    You may need to adapt this path on your machine.
    """
    candidate_paths = [
    ...
    for path in candidate_paths:
        if os.path.exists(path):
            try:
                ...
                print(f"[WARN] 获取 CPU 温度失败 ({path}): {e}")
    print("[WARN] 未找到可用的 CPU 温度路径")
    return None
...
def get_gpu_temperature() -> Optional[float]:
    try:
        ...
        print(f"[WARN] 获取 GPU 温度失败: {e}")
        return None


class CPUUsageSampler:
    """
    Compute instantaneous CPU usage from /proc/stat deltas.
    """
    def __init__(self):
...
def compute_alpha_t(
    ...
    """
    Temperature-aware attenuation factor.

    alpha_t = 1                              if T <= threshold
            = 1 - min(over_temp_ratio * PRR_max, PRR_max)   otherwise

    This ensures:
    - alpha_t in [1-PRR_max, 1]
    - higher temperature => lower alpha_t
    """
...
class CPUThermalController:
    ...
    def set_governor_userspace_or_powersave(self):
        """
        Try to use a governor that respects scaling_max_freq better.
        """
        for info in self.cpu_infos:
            ...
            try:
                ...
                if current is None:
                    continue
                # Prefer powersave for broad compatibility
                self._write_str(gov_path, "powersave")
            except Exception as e:
                print(f"[WARN] 设置 CPU{info['cpu_id']} governor 失败: {e}")

    def apply_alpha(self, alpha_t: float) -> Optional[float]:
        """
        Apply real CPU throttling by reducing scaling_max_freq.
        Returns average target max freq in MHz.
        """
        ...
        for info in self.cpu_infos:
            ...
            try:
                ...
                print(f"[WARN] 设置 CPU{cpu_id} 最大频率失败: {e}")

        if not target_freqs:
            return None

        return sum(target_freqs) / len(target_freqs) / 1000.0  # kHz -> MHz
...
class GPUThermalController:
    ...
    def capture_original_state(self):
        ...
        try:
            ...
            print(f"[WARN] 读取 GPU power limit 信息失败: {e}")

    def apply_alpha(self, alpha_t: float) -> Optional[float]:
        """
        Apply real GPU throttling by reducing GPU power limit.
        """
        if not self.available():
            return None
        ...
        try:
            ...
            print(f"[WARN] 设置 GPU power limit 失败: {e}")
            return None
    ...
    def restore(self):
        ...
        try:
            ...
            print(f"[WARN] 恢复 GPU power limit 失败: {e}")
...
class ThermalThrottleManager:
    ...
    def loop(self):
        ...
        with open(self.log_csv_path, "w", newline="") as csvfile:
            ...
            # warm-up sample for CPU usage delta
...
def main():
    ...
    def cleanup_and_exit(signum=None, frame=None):
        ...
        try:
            ...
            print(f"[WARN] 结束子进程失败: {e}")

        try:
            manager.stop()
        except Exception as e:
            print(f"[WARN] 恢复控制器状态失败: {e}")
    ...
    try:
        ...
        print(f"[INFO] 启动目标程序: {target_script}")
        child = subprocess.Popen(["python3", target_script])

        return_code = child.wait()
        print(f"[INFO] 目标程序已结束，返回码: {return_code}")

    finally:
        cleanup_and_exit()
        if monitor_thread is not None and monitor_thread.is_alive():
            monitor_thread.join(timeout=5)
        print("[INFO] 热节流监控已结束，原始配置已恢复。")
